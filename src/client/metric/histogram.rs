...
impl Histogram {
    ...
    fn update_instances(&mut self) -> io::Result<()> {
        self.im
            .set_val(MIN_INST, self.histogram.min() as f64)
            .unwrap()?;
        self.im
            .set_val(MAX_INST, self.histogram.max() as f64)
            .unwrap()?;
    ...
    pub fn hdr_histogram(&self) -> &HdrHist<u64> {
        &self.histogram
    }
}
...
pub fn test() {
    ...
    assert_eq!(*hist.im.val(MEAN_INST).unwrap(), hist.histogram.mean());

    assert_eq!(*hist.im.val(STDEV_INST).unwrap(), hist.histogram.stdev());
}
